...
def model_fn(features, mode, gpt_config, vocab, init_checkpoint, learning_rate,
    ...
    with tf.variable_scope("MLP", reuse=tf.AUTO_REUSE):
        ...
        # input_mask
        print("============================")
        mask_2d = tf.tile(tf.expand_dims(input_mask, axis=2), multiples=[1, 1, gpt_config.hidden_size])
        print(mask_2d)
...
def main(_):
    ...
    train_gpt_model, train_op, total_loss = model_fn(train_features, mode, gpt_config, vocab,
    ...
    while True:
        try:
            ...
            saver.save(sess, os.path.join(FLAGS.checkpointDir, "gpt_finetune_" + FLAGS.bizdate))
            break
    ...
    test_gpt_model, y_hat, total_loss = model_fn(test_features, mode, gpt_config, vocab,
                                                  os.path.join(FLAGS.checkpointDir, "gpt_finetune_" + FLAGS.bizdate),
                                                  FLAGS.learning_rate,
                                                  FLAGS.num_train_steps, FLAGS.num_warmup_steps, False, False)
